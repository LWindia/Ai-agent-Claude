...
export default function OutcomesSection() {
  return (
    <section id="outcomes" aria-label="Course Outcomes – What You'll Be Capable Of" className="py-14 px-4 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <AnimatedSection>
          <div className="flex flex-col items-center mb-10">
            <SectionHeading
              ...
              subtitle="Graduate with AI-native DevOps skills that put you ahead of 99% of engineers."
            />
          </div>
        </AnimatedSection>
          ...
          {outcomes.map((item, i) => (
            <AnimatedSection key={item.title} speed={i % 2 === 0 ? 'normal' : 'slow'}>
              ...
              <GlassCard glowOnHover glowColor="primary" className="p-6 flex flex-col gap-4 h-full group relative overflow-hidden">
                <div aria-hidden="true" className="absolute top-0 right-0 w-24 h-24 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none"
                  style={{ background: 'radial-gradient(circle at top right, rgba(217,119,87,0.12), transparent 70%)' }} />
                <div aria-hidden="true" className="w-12 h-12 rounded-xl flex items-center justify-center text-xl transition-all duration-300 group-hover:scale-110"
                ...
                <div aria-hidden="true" className="mt-auto h-0.5 w-0 group-hover:w-full transition-all duration-500 rounded-full"
        ...
        <AnimatedSection speed="slow">
          <div className="mt-10 glass rounded-2xl p-6 grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
            {[
              { value: '20', label: 'Days', color: '#D97757' },
              { value: '40+', label: 'Hours of Content', color: '#C4623E' },
              { value: '2,400+', label: 'Students Enrolled', color: '#D97757' },
              { value: '4.9★', label: 'Average Rating', color: '#C4623E' },
            ].map((stat) => (
              <div key={stat.label}>
